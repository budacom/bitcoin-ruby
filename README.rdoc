...
* BitcoinOld::Key provides a high-level API for creating and handling keys/addresses
* BitcoinOld::Util provides the basic bitcoin utility functions for base58, ECC, etc.
* BitcoinOld::Protocol can parse/create all protocol messages
* BitcoinOld::Script implementation, create/run scripts and verify signatures
* BitcoinOld::Builder provides a high-level API for creating transactions (and blocks)
* BitcoinOld::Litecoin implements all the litecoin-specific differences
...
Generate a BitcoinOld::Key
 ...
 sig = key.sign("data")
 key.verify("data", sig)
 recovered_key = BitcoinOld::Key.from_base58(key.to_base58)

=== Blocks / Transactions parsing

Parse a BitcoinOld::Protocol::Block

 raw_block = File.open('spec/bitcoin/fixtures/rawblock-0.bin', 'rb') {|f| f.read}
 blk = BitcoinOld::Protocol::Block.new(raw_block)
 blk.hash #=> 00000000839a8e6886ab5951d76f411475428afc90947ee320161bbf18eb6048
 blk.tx.count #=> 1
 blk.to_hash #=> ...
 BitcoinOld::Protocol::Block.from_json( blk.to_json )

Parse a BitcoinOld::Protocol::Tx

 raw_tx = File.open('spec/bitcoin/fixtures/rawtx-01.bin', 'rb') {|f| f.read}
 tx = BitcoinOld::Protocol::Tx.new(raw_tx)
 tx.hash #=> 6e9dd16625b62cfcd4bf02edb89ca1f5a8c30c4b1601507090fb28e59f2d02b4
 tx.in.size #=> 1
 tx.out.size #=> 2
 tx.to_hash #=> ...
 BitcoinOld::Protocol::Tx.from_json( tx.to_json )

 BitcoinOld::Script.new(tx.out[0].pk_script).to_string
...
Get the matching transactions (in this example tx1 is the spending transaction)
 ...
 tx1 = BitcoinOld::Protocol::Tx.new(rawtx1)
 tx2 = BitcoinOld::Protocol::Tx.new(rawtx2)
...
If you want to control the BitcoinOld::Script yourself, you can do so

 txin = tx1.in.first
 txout = tx2.out[txin.prev_out_index]
 script = BitcoinOld::Script.new(txin.script_sig + txout.pk_script)
...
as well as the private key for the address required to sign for it.
 ...
 prev_tx = BitcoinOld::P::Tx.from_json(open("http://test.webbtc.com/tx/#{prev_hash}.json"))
 
 # the key needed to sign an input that spends the previous output
 key = BitcoinOld::Key.from_base58("92ZRu28m2GHSKaaF2W7RswJ2iJYpTzVhBaN6ZLs7TENCs4b7ML8")
