...
describe 'BitcoinOld::Protocol::Parser (notfound)' do
  # Mock handler
  class NotfoundHandler < BitcoinOld::Protocol::Handler
  ...
  it 'parses notfound block message' do
    ...
    expect(
      parser.parse(BitcoinOld::Protocol.pkt('notfound', payload) + 'AAAA')
    ).to eq('AAAA')
