...
describe 'BitcoinOld::Protocol::Parser (addr)' do
  # Mock protocl handler
  class AddrHandler < BitcoinOld::Protocol::Handler
  ...
  it 'parses address packet' do
    ...
    parser = BitcoinOld::Protocol::Parser.new(handler)
...
describe BitcoinOld::Protocol::Addr do
  let(:pkt) do
    [
  ...
  it 'pack addr packet' do
    ...
    addr[:ip] = '82.83.222.4'
    addr[:port] = 8_333
    expect(BitcoinOld::Protocol::Addr.pkt(addr))
      .to eq(BitcoinOld::Protocol.pkt('addr', "\x01" + addr.to_payload))
  end
end
