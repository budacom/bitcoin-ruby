...
describe 'BitcoinOld::Protocol::Parser - Inventory Vectors' do
  # Protocol handler that exposes information required from tests.
  class TestProtocolHandler < BitcoinOld::Protocol::Handler
  ...
  it 'parses getblocks' do
    ...
    pkt = BitcoinOld::Protocol.getblocks_pkt(
      BitcoinOld.network[:protocol_version],
      locator_hashes
    )
    ...
    expect(handler.pkt).to eq(
      version: BitcoinOld.network[:protocol_version],
  ...
  it 'parses getheaders' do
    ...
    pkt = BitcoinOld::Protocol.getblocks_pkt(
      BitcoinOld.network[:protocol_version], locator_hashes, stop_hash
    )
