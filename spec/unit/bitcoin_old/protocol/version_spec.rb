...
describe 'BitcoinOld::Protocol::Parser (version)' do
  # Mock interface
  class VersionHandler < BitcoinOld::Protocol::Handler
    attr_reader :pkt
  ...
  it 'parses version packets' do
    ...
    expect(pkt.fields)
      .to eq(version: 60_000,
             services: BitcoinOld::Protocol::Version::NODE_NETWORK,
             time: 1_329_775_027,
  ...
  it 'creates version packets' do
    ...
    pkt = handler.pkt
    expect(pkt.fields)
      .to eq(version: BitcoinOld.network[:protocol_version],
             services: BitcoinOld::Protocol::Version::NODE_NETWORK,
             time: 1_337,
             to: '127.0.0.1:8333',
             from: '127.0.0.1:1234',
             nonce: 123,
             user_agent: "/bitcoin-ruby:#{BitcoinOld::VERSION}/",
             last_block: 188_617,
             relay: true)
  end
