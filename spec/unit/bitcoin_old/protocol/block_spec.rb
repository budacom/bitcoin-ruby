...
describe BitcoinOld::Protocol::Block do
  let(:blocks) do
    {
      # block 0:
  ...
  let(:block) { BitcoinOld::Protocol::Block.new(blocks['0']) }

  it '#new' do
    BitcoinOld::Protocol::Block.new(nil)
    BitcoinOld::Protocol::Block.new(blocks['0'])

    expect do
      BitcoinOld::Protocol::Block.new(blocks['0'][0..20])
    end.to raise_error(TypeError, "can't convert nil into Integer")

    block = BitcoinOld::Protocol::Block.new(nil)
    ...
    block = BitcoinOld::Protocol::Block.new(blocks['testnet-1151351'])
  ...
  it '#tx' do
    ...
    expect(block.tx[0].is_a?(BitcoinOld::Protocol::Tx)).to be true
  ...
  it '#to_json' do
    ...
    expect(BitcoinOld::Protocol::Block.new(blocks['1']).to_json)
      .to eq(fixtures_file('rawblock-1.json'))
    expect(BitcoinOld::Protocol::Block.new(blocks['131025']).to_json)
      .to eq(fixtures_file('rawblock-131025.json'))
    expect(BitcoinOld::Protocol::Block.new(blocks['testnet-26478']).to_json)
      .to eq(fixtures_file('rawblock-testnet-26478.json'))
    expect(BitcoinOld::Protocol::Block.from_json(block.to_json).tx[0].in[0].sequence)
      .to eq("\xff\xff\xff\xff")
  end

  it '#to_payload' do
    expect(block.to_payload).to eq(block.payload)
    expect(BitcoinOld::Protocol::Block.new(block.to_payload).to_payload)
      .to eq(block.payload)
    expect(BitcoinOld::Protocol::Block.new(blocks['1']).to_payload)
      .to eq(blocks['1'])
    expect(BitcoinOld::Protocol::Block.new(blocks['131025']).to_payload)
      .to eq(blocks['131025'])
    expect(BitcoinOld::Protocol::Block.new(blocks['testnet-26478']).to_payload)
      .to eq(blocks['testnet-26478'])
  end

  describe '.from_json' do
    it 'should load blocks from json' do
      block = BitcoinOld::Protocol::Block.from_json(
        fixtures_file('rawblock-0.json')
      )
      expect(block.to_payload).to eq(blocks['0'])
      expect(block.tx[0].in[0].sequence).to eq("\xff\xff\xff\xff")
      expect(
        BitcoinOld::Protocol::Block.from_json(
      ...
      expect(BitcoinOld::Protocol::Block.new(block_raw).to_payload)
        .to eq(block_raw)
    end
    ...
    it 'should work with litecoin blocks' do
      ...
      json = BitcoinOld::Protocol::Block.new(
        fixtures_file("#{litecoin_block}.bin")
      ).to_json
      expect(BitcoinOld::Protocol::Block.from_json(json).to_payload)
        .to eq(fixtures_file("#{litecoin_block}.bin"))
      expect(BitcoinOld::Protocol::Block.from_json(json).hash)
    ...
    it 'should check block hash' do
      ...
      expect do
        BitcoinOld::Protocol::Block.from_hash(h)
      end.to raise_error(
        RuntimeError,
  ...
  it 'filtered block parsing' do
    block = BitcoinOld::Protocol::Block.new
...
JSON
  ...
  it '#verify_mrkl_root' do
    block0 = BitcoinOld::Protocol::Block.from_json(
      fixtures_file('rawblock-0.json')
    )
    block1 = BitcoinOld::Protocol::Block.from_json(
      fixtures_file('rawblock-1.json')
    )
  ...
  it 'should work with huge block version' do
    expect(BitcoinOld::Protocol::Block.new(blocks['testnet-265322']).hash)
