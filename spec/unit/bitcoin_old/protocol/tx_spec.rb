...
describe BitcoinOld::Protocol::Tx do
  let(:payloads) do
    [
  ...
  describe '#new' do
    it 'does not raise an error for valid payloads' do
      BitcoinOld::Protocol::Tx.new(nil)
      payloads.each { |payload| BitcoinOld::Protocol::Tx.new(payload) }
    end

    it 'raises an error for an invalid payload' do
      expect do
        BitcoinOld::Protocol::Tx.new(payloads[0][0..20])
      end.to raise_error(NoMethodError, /undefined method `unpack'/)
    end

    it 'correctly deserializes a new, empty transaction' do
      BitcoinOld::Protocol::Tx.new(BitcoinOld::Protocol::Tx.new.to_payload)
    end
  end

  describe '#parse_data' do
    let(:tx) { BitcoinOld::Protocol::Tx.new(nil) }
  ...
  describe '#parse_witness_data' do
    it 'correctly parses witness data' do
      tx = BitcoinOld::Protocol::Tx.new(payloads[3])
      expect(tx.hash.size).to eq(64)
      expect(tx.payload).to eq(payloads[3])
    end

    it 'ignores additional payload data' do
      tx = BitcoinOld::Protocol::Tx.new(payloads[3] + 'AAAA')
      expect(tx.hash.size).to eq(64)
      expect(tx.payload).to eq(payloads[3])
    end
  end
  ...
  describe '#hash' do
    it 'produces the expected hash and binary hash' do
      tx = BitcoinOld::Protocol::Tx.new(payloads[0])
      expect(tx.hash.size).to eq(64)
      expect(tx.hash)
  ...
  describe '#normalized_hash' do
    it 'produces the expected normalized hash' do
      ...
      script = BitcoinOld::Script.from_string(new_tx['in'][0]['scriptSig'])
      script.chunks[0].bitcoin_pushdata = BitcoinOld::Script::OP_PUSHDATA2
      script.chunks[0].bitcoin_pushdata_length = script.chunks[0].bytesize
      new_tx['in'][0]['scriptSig'] = script.to_string
      new_tx = BitcoinOld::Protocol::Tx.from_hash(new_tx, false)
  ...
  it '.from_hash' do
    orig_tx = BitcoinOld::Protocol::Tx.new(payloads[0])
    tx = BitcoinOld::Protocol::Tx.from_hash(orig_tx.to_hash)
    expect(tx.payload).to eq(payloads[0])
    expect(tx.to_payload.size).to eq(payloads[0].size)
    expect(tx.to_payload).to eq(payloads[0])
    expect(tx.to_hash).to eq(orig_tx.to_hash)
    expect(BitcoinOld::Protocol::Tx.binary_from_hash(orig_tx.to_hash))
      .to eq(payloads[0])

    h = orig_tx.to_hash.merge('ver' => 123)
    expect do
      BitcoinOld::Protocol::Tx.from_hash(h)
    ...
    # witness tx(P2WPKH)
    orig_tx = BitcoinOld::Protocol::Tx.new(payloads[3])
    tx = BitcoinOld::Protocol::Tx.from_hash(orig_tx.to_hash)
    expect(tx.payload).to eq(payloads[3])
    expect(tx.to_witness_payload.size).to eq(payloads[3].size)
    expect(tx.to_witness_payload).to eq(payloads[3])
  ...
  it '.binary_from_hash' do
    orig_tx = BitcoinOld::Protocol::Tx.new(payloads[0])
    expect(BitcoinOld::Protocol::Tx.binary_from_hash(orig_tx.to_hash).size).to eq(payloads[0].size)
    expect(BitcoinOld::Protocol::Tx.binary_from_hash(orig_tx.to_hash)).to eq(payloads[0])

    orig_tx = BitcoinOld::Protocol::Tx.new(payloads[3])
    expect(BitcoinOld::Protocol::Tx.binary_from_hash(orig_tx.to_hash).size).to eq(payloads[3].size)
    expect(BitcoinOld::Protocol::Tx.binary_from_hash(orig_tx.to_hash)).to eq(payloads[3])
  end
  ...
  it '#to_json' do
    ...
    tx = BitcoinOld::Protocol::Tx.new(payloads[1])
    expect(tx.to_json).to eq(json[1])

    tx = BitcoinOld::Protocol::Tx.new(payloads[2])
  ...
  it '.from_json' do
    ...
    tx = BitcoinOld::Protocol::Tx.from_json(json_string)
    ...
    tx = BitcoinOld::Protocol::Tx.from_json(
      fixtures_file(
        'rawtx-ba1ff5cd66713133c062a871a8adab92416f1e38d17786b2bf56ac5f6ffdfdf5.json'
      )
    )
    expect(BitcoinOld::Protocol::Tx.new(tx.to_payload).to_json)
      .to eq(tx.to_json)
    expect(tx.hash)
    ...
    tx = BitcoinOld::Protocol::Tx.from_json(json_string)
    expect(BitcoinOld::Protocol::Tx.new(tx.to_payload).to_json).to eq(json_string)

    # TODO: Deprecate this since Toshi is no longer maintained?
    # toshi format
    expect(
      BitcoinOld::Protocol::Tx.from_json(
        fixtures_file('rawtx-02-toshi.json')
      ).to_payload
    ).to eq(
      BitcoinOld::Protocol::Tx.from_json(fixtures_file('rawtx-02.json')).to_payload
    )
    expect(
      BitcoinOld::Protocol::Tx.from_json(
      ...
      BitcoinOld::Protocol::Tx.from_json(fixtures_file('rawtx-03.json')).to_payload
    )
    expect(
      BitcoinOld::Protocol::Tx.from_json(
      ...
      BitcoinOld::Protocol::Tx.from_json(fixtures_file('coinbase.json')).to_payload
    )
  ...
  it '.binary_from_json' do
    ...
    expect(BitcoinOld::Protocol::Tx.binary_from_json(json_string))
      .to eq(binary_string)
  end
  ...
  describe '.compare_big_endian' do
    it 'compares arrays of bytes' do
      ...
      c1 = []
      c2 = []
      expect(BitcoinOld::Script.compare_big_endian(c1, c2)).to eq(0)
      ...
      c1 = [4]
      c2 = [5]
      expect(BitcoinOld::Script.compare_big_endian(c1, c2)).to eq(-1)
  ...
  describe '.is_der_signature?' do
    it 'validates ECDSA signature format' do
      ...
      expect(BitcoinOld::Script.is_der_signature?(sig_orig)).to be true
      expect(BitcoinOld::Script.is_defined_hashtype_signature?(sig_orig)).to be true

      # Trimmed to be too short
      sig = sig_orig.slice(0, 8)
      expect(BitcoinOld::Script.is_der_signature?(sig)).to be false
  ...
  it '#verify_input_signature' do
    ...
    outpoint_tx = BitcoinOld::Protocol::Tx.new(
      fixtures_file(
    ...
    outpoint_tx = BitcoinOld::Protocol::Tx.from_json(
      fixtures_file(
    ...
    tx.in.map(&:previous_output).uniq.each do |i|
      prev_txs[i] = BitcoinOld::Protocol::Tx.from_json(
  ...
  describe '#signature_hash_for_input' do
    it 'sighash_all' do
      ...
      key = BitcoinOld.open_key(
        '56e28a425a7b588973b5db962a09b1aca7bdc4a7268cdd671d03c52a997255dc',
        pubkey
      )
      new_tx = BitcoinOld::Protocol::Tx.new(nil)
      new_tx.add_in(BitcoinOld::Protocol::TxIn.new(prev_tx.binary_hash, 0, 0))
      new_tx.add_out(
        BitcoinOld::Protocol::TxOut.value_to_address(
          1_000_000,
          '1BVJWLTCtjA8wRivvrCiwjNdL6KjdMUCTZ'
        )
      )
      signature_hash = new_tx.signature_hash_for_input(0, prev_tx)
      sig = BitcoinOld.sign_data(key, signature_hash)
      new_tx.in[0].script_sig = BitcoinOld::Script.to_pubkey_script_sig(
        sig, [pubkey].pack('H*')
      )

      new_tx = BitcoinOld::Protocol::Tx.new(new_tx.to_payload)
      ...
      new_tx = BitcoinOld::Protocol::Tx.new(nil)
      new_tx.add_in(BitcoinOld::Protocol::TxIn.new(prev_tx.binary_hash, 0, 0))
      pk_script = BitcoinOld::Script.to_address_script(
        '1FEYAh1x5jeKQMPPuv3bKnKvbgVAqXvqjW'
      )
      new_tx.add_out(
        BitcoinOld::Protocol::TxOut.new(1_000_000, pk_script)
      )
      signature_hash = new_tx.signature_hash_for_input(0, prev_tx)
      sig = BitcoinOld.sign_data(key, signature_hash)
      new_tx.in[0].script_sig = BitcoinOld::Script.to_pubkey_script_sig(
      ...
      new_tx = BitcoinOld::Protocol::Tx.new(nil)
      new_tx.add_in(
        BitcoinOld::Protocol::TxIn.new(prev_tx.binary_hash, 0, 0)
      )
      new_tx.add_out(
        BitcoinOld::Protocol::TxOut.value_to_address(
    ...
    it 'sighash JSON tests' do
      ...
      test_cases.each do |test_case|
        ...
        transaction = BitcoinOld::Protocol::Tx.new(test_case[0].htb)
  ...
  it '#signature_hash_for_witness_input' do
    ...
    signature_hash = tx.signature_hash_for_witness_input(
      ...
      witness_script.htb, BitcoinOld::Protocol::Tx::SIGHASH_TYPE[:single]
    )
    expect(signature_hash.bth)
  ...
  it '#legacy_sigops_count' do
    expect(BitcoinOld::Protocol::Tx.new(payloads[0]).legacy_sigops_count).to eq(2)
    expect(BitcoinOld::Protocol::Tx.new(payloads[1]).legacy_sigops_count).to eq(2)
    expect(BitcoinOld::Protocol::Tx.new(payloads[2]).legacy_sigops_count).to eq(2)

    # Test sig ops count in inputs too.
    tx = BitcoinOld::Protocol::Tx.new
    txin = BitcoinOld::Protocol::TxIn.new
    txin.script_sig = BitcoinOld::Script.from_string(
      '10 OP_CHECKMULTISIGVERIFY OP_CHECKSIGVERIFY'
    ).to_binary
    tx.add_in(txin)
    txout = BitcoinOld::Protocol::TxOut.new
    txout.pk_script = BitcoinOld::Script.from_string(
  ...
  describe 'Tx - final?' do
    ...
    it 'should be final if lock_time is below timestamp' do
      ...
      txin.sequence = "\x01\x00\x00\x00"
      tx.add_in(txin)
      tx.lock_time = BitcoinOld::LOCKTIME_THRESHOLD # when equal, interpreted as threshold
      expect(tx.final?(0, BitcoinOld::LOCKTIME_THRESHOLD - 1)).to be false
      # when equal to timestamp, still not final
      expect(tx.final?(0, BitcoinOld::LOCKTIME_THRESHOLD)).to be false
      expect(tx.final?(0, BitcoinOld::LOCKTIME_THRESHOLD + 1)).to be true

      tx.lock_time = BitcoinOld::LOCKTIME_THRESHOLD + 666
      expect(tx.final?(0, BitcoinOld::LOCKTIME_THRESHOLD + 1)).to be false
      # when equal to timestamp, still not final
      expect(tx.final?(0, BitcoinOld::LOCKTIME_THRESHOLD + 666)).to be false
      expect(tx.final?(0, BitcoinOld::LOCKTIME_THRESHOLD + 667)).to be true
    end
    ...
    it 'should be final if all inputs are finalized regardless of lock_time' do
      ...
      tx.lock_time = BitcoinOld::LOCKTIME_THRESHOLD
      expect(tx.final?(0, BitcoinOld::LOCKTIME_THRESHOLD - 1)).to be true
      expect(tx.final?(0, BitcoinOld::LOCKTIME_THRESHOLD)).to be true
      expect(tx.final?(0, BitcoinOld::LOCKTIME_THRESHOLD + 1)).to be true

      tx.lock_time = BitcoinOld::LOCKTIME_THRESHOLD + 666
      expect(tx.final?(0, BitcoinOld::LOCKTIME_THRESHOLD + 1)).to be true
      expect(tx.final?(0, BitcoinOld::LOCKTIME_THRESHOLD + 666)).to be true
      expect(tx.final?(0, BitcoinOld::LOCKTIME_THRESHOLD + 667)).to be true
    end
  end
  ...
  it '#calculate_minimum_fee for litecoin' do
    ...
    BitcoinOld.network = :litecoin # change to litecoin
    expect(tx.minimum_relay_fee).to eq(0)
    expect(tx.minimum_block_fee).to eq(100_000)
  end

  it 'should compare transactions' do
    tx1 = BitcoinOld::Protocol::Tx.new(payloads[0])
    tx2 = BitcoinOld::Protocol::Tx.new(payloads[1])

    expect(BitcoinOld::Protocol::Tx.from_json(tx1.to_json)).to eq(tx1)
  ...
  describe 'Tx - BIP Scripts' do
    it 'should do OP_CHECKMULTISIG' do
      ...
      prev_tx = BitcoinOld::Protocol::Tx.from_json(
        fixtures_file(
  ...
  it 'lexicographical_sort' do
    ...
    tx = BitcoinOld::Protocol::Tx.new
    tx.add_out(BitcoinOld::Protocol::TxOut.new(500, 'bbbbbbbb'.htb))
    tx.add_out(BitcoinOld::Protocol::TxOut.new(500, 'aaaaaaaa'.htb))
    tx.add_out(BitcoinOld::Protocol::TxOut.new(500, 'cccccccc'.htb))
  ...
  describe 'verify_input_signature' do
    ...
    def parse_script(script_str)
      ...
      script_str.split.each do |token|
        opcode = BitcoinOld::Script::OPCODES_PARSE_STRING[token] ||
                 BitcoinOld::Script::OPCODES_PARSE_STRING['OP_' + token]
        if opcode
          buf << [opcode].pack('C')
          next
        end
        ...
        data =
          ...
          when /\A-?\d+\z/
            ...
            opcode =
              case i
              when -1 then BitcoinOld::Script::OP_1NEGATE
              when 0 then BitcoinOld::Script::OP_0
              when 1 then BitcoinOld::Script::OP_1
              when 2..16 then BitcoinOld::Script::OP_2 + i - 2
              end

            if opcode
              [opcode].pack('C')
            else
              BitcoinOld::Script.pack_pushdata(script.cast_to_string(i))
            end
          when /\A'(.*)'\z/ then
            BitcoinOld::Script.pack_pushdata(Regexp.last_match(1))
    ...
    it 'script JSON tests' do
      ...
      test_cases.each do |test_case|
        ...
        crediting_tx = BitcoinOld::Protocol::Tx.new
        crediting_tx.add_in(BitcoinOld::Protocol::TxIn.new)
        crediting_tx.in[0].prev_out_hash = BitcoinOld::Protocol::TxIn::NULL_HASH
        crediting_tx.in[0].prev_out_index = BitcoinOld::Protocol::TxIn::COINBASE_INDEX
        crediting_tx.in[0].script_sig = parse_script('0 0')
        crediting_tx.add_out(BitcoinOld::Protocol::TxOut.new)
        crediting_tx.out[0].value = value
        crediting_tx.out[0].pk_script = script_pubkey
        crediting_tx.refresh_hash

        spending_tx = BitcoinOld::Protocol::Tx.new
        spending_tx.add_in(BitcoinOld::Protocol::TxIn.new)
        ...
        spending_tx.add_out(BitcoinOld::Protocol::TxOut.new)
