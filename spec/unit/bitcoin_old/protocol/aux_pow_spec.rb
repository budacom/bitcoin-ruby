...
describe BitcoinOld::Protocol::AuxPow do
  before { BitcoinOld.network = :namecoin }
  let(:data) { fixtures_file('rawblock-auxpow.bin') }
  let(:blk) { BitcoinOld::Protocol::Block.new(data) }
  ...
  it '#to_payload' do
    ...
    expect(BitcoinOld::Protocol::Block.new(blk.to_payload).to_payload).to eq(data)
  end

  it '#to_hash' do
    expect(BitcoinOld::Protocol::Block.from_hash(blk.to_hash).to_payload).to eq(data)
  end

  it '#to_json' do
    expect(BitcoinOld::Protocol::Block.from_json(blk.to_json).to_payload).to eq(data)
  end
end
