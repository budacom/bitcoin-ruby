...
describe 'BIP143 spec' do
  ...
  it 'Native P2WPKH' do
    ...
    sig0 = BitcoinOld::Secp256k1.sign(
      sig_hash0,
      'bbc27228ddcb9209d7fd6f36b02f7dfa6252af40bb2f1cbc7a557da8027ff866'.htb
    ) + [BitcoinOld::Protocol::Tx::SIGHASH_TYPE[:all]].pack('C')

    tx.in[0].script_sig = BitcoinOld::Script.new(
      BitcoinOld::Script.pack_pushdata(sig0)
    ).to_payload
    ...
    sig1 = BitcoinOld::Secp256k1.sign(
      sig_hash1,
  ...
  it 'P2SH-P2WPKH' do
    ...
    tx.in[0].script_sig = BitcoinOld::Script.new(
      BitcoinOld::Script.pack_pushdata(redeem_script.htb)
    ).to_payload

    sig_hash = tx.signature_hash_for_witness_input(
      0, redeem_script.htb, 1_000_000_000
    )
    sig = BitcoinOld::Secp256k1.sign(
      sig_hash,
  ...
  it 'Native P2WSH' do
    ...
    witness_script = BitcoinOld::Script.new(
    ...
    sig_hash1 = tx.signature_hash_for_witness_input(
      ...
      BitcoinOld::Protocol::Tx::SIGHASH_TYPE[:single]
    )
    sig1 = BitcoinOld::Secp256k1.sign(
      sig_hash1,
      '8e02b539b1500aa7c81cf3fed177448a546f19d2be416c0c61ff28e577d8d0cd'.htb
    ) + [BitcoinOld::Protocol::Tx::SIGHASH_TYPE[:single]].pack('C')
    ...
    sig2 = BitcoinOld::Secp256k1.sign(
      sig_hash2,
  ...
  it 'P2SH-P2WSH' do
    ...
    sig_hash1 = tx.signature_hash_for_witness_input(
      ...
      987_654_321,
      witness_script.to_payload,
      BitcoinOld::Protocol::Tx::SIGHASH_TYPE[:none]
    )
    sig1 = BitcoinOld::Secp256k1.sign(
      sig_hash1, '11fa3d25a17cbc22b29c44a484ba552b5a53149d106d3d853e22fdd05a2d8bb3'.htb
    ) + [BitcoinOld::Protocol::Tx::SIGHASH_TYPE[:none]].pack('C')
    expect(sig1.bth)
    ...
    sig_hash3 = tx.signature_hash_for_witness_input(
      ...
      987_654_321,
      witness_script.to_payload,
      BitcoinOld::Protocol::Tx::SIGHASH_TYPE[:all] | BitcoinOld::Protocol::Tx::SIGHASH_TYPE[:anyonecanpay]
    )
    sig3 = BitcoinOld::Secp256k1.sign(
      sig_hash3,
      '14af36970f5025ea3e8b5542c0f8ebe7763e674838d08808896b63c3351ffe49'.htb
    ) + [
      BitcoinOld::Protocol::Tx::SIGHASH_TYPE[:all] |
      BitcoinOld::Protocol::Tx::SIGHASH_TYPE[:anyonecanpay]
    ].pack('C')
    expect(sig3.bth)
    ...
    sig4 = BitcoinOld::Secp256k1.sign(
      sig_hash4,
    ...
    sig5 = BitcoinOld::Secp256k1.sign(
      sig_hash5,
