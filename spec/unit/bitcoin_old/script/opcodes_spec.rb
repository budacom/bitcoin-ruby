...
describe BitcoinOld::Script do
  ...
  let(:script) { BitcoinOld::Script.new('') }
  ...
  def run_script(string, hash)
    script = BitcoinOld::Script.from_string(string)
    script.run do |pk, sig, _|
      begin
        k = BitcoinOld::Key.new(nil, pk.unpack('H*')[0])
        k && k.verify(hash, sig)
  ...
  it 'should do if/notif/else/end' do
    [
      ...
      expect(BitcoinOld::Script.from_string(script).run).to be true
    end
  end
  ...
  it 'should do OP_CHECKSIG' do
    ...
    verify_callback = proc do |pub, sig, hash_t|
      ...
      hash = signature_data
      BitcoinOld.verify_signature(hash, sig, pub.unpack('H*')[0])
    end
    expect(script.op_checksig(verify_callback)).to eq([1])
    ...
    verify_callback = proc do |pub, _, _|
      hash = signature_data
      BitcoinOld.verify_signature(hash, 'foo', pub.unpack('H*')[0])
    end
    expect(script.op_checksig(verify_callback)).to eq([0])
    ...
    verify_callback = proc do |_, sig, _|
      hash = signature_data
      BitcoinOld.verify_signature(hash, sig, 'foo')
    end
    expect(script.op_checksig(verify_callback)).to eq([0])
    ...
    script = BitcoinOld::Script.from_string("#{signature} #{key.pub} OP_CHECKSIG")
  ...
  it 'should do OP_CHECKMULTISIG' do
    k1 = BitcoinOld::Key.new
    k2 = BitcoinOld::Key.new
    k3 = BitcoinOld::Key.new
    k1.generate
    k2.generate
    k3.generate
    ...
    expect(run_script(script, 'foobar')).to be true
    expect(BitcoinOld::Script.from_string(script).get_addresses).to be_empty
    expect(BitcoinOld::Script.from_string(script).is_multisig?).to be false
  ...
  it 'should do P2SH' do
    ...
    inner_script = BitcoinOld::Script.from_string("#{k1.pub} OP_CHECKSIG").raw.unpack('H*')[0]
    script_hash = BitcoinOld.hash160(inner_script)
    ...
    script = BitcoinOld::Script.from_string("OP_HASH160 #{script_hash} OP_EQUAL")
    expect(script.is_p2sh?).to be true
    expect(run_script(script.to_string, 'foobar')).to be false

    address = '3CkxTG25waxsmd13FFgRChPuGYba3ar36B'
    script = BitcoinOld::Script.new(BitcoinOld::Script.to_address_script(address))
    expect(script.type).to eq(:p2sh)

    inner_script = BitcoinOld::Script.from_string('0 OP_NOT').raw.unpack('H*')[0]
    script_hash = BitcoinOld.hash160(inner_script)
    script = BitcoinOld::Script.from_string("#{inner_script} OP_HASH160 #{script_hash} OP_EQUAL")
    expect(script.is_p2sh?).to be true
    expect(run_script(script.to_string, 'foobar')).to be true
  end

  it 'should skip OP_EVAL' do
    expect(BitcoinOld::Script.from_string('1 OP_EVAL').to_string).to eq('1 OP_NOP1')
    expect(BitcoinOld::Script.from_string('1 OP_EVAL').run).to be true
    expect(BitcoinOld::Script.from_string('0 OP_EVAL').run).to be false
  end
  ...
  it 'should do testnet3 scripts' do
    [
      ...
      parsed_script = BitcoinOld::Script.from_string(script)
  ...
  it 'should do OP_VER' do
    s = BitcoinOld::Script.from_string('OP_VER')
    s.run
    expect(s).to be_invalid

    s = BitcoinOld::Script.from_string('1 OP_IF OP_VER 1 OP_ELSE 0 OP_ENDIF')
    expect(s.run).to be false
    expect(s).to be_invalid

    s = BitcoinOld::Script.from_string('1 OP_IF 1 OP_ELSE OP_VER 0 OP_ENDIF')
    expect(s.run).to be true
    expect(s).not_to be_invalid
  end

  it 'should not allow DISABLED_OPCODES' do
    BitcoinOld::Script::DISABLED_OPCODES.each do |opcode|
      s = BitcoinOld::Script.from_string(BitcoinOld::Script::OPCODES[opcode] + ' 1')
      expect(s.run).to be false
      expect(s).to be_invalid

      s = BitcoinOld::Script.from_string(
        "1 OP_IF #{BitcoinOld::Script::OPCODES[opcode]} 1 OP_ELSE 1 OP_ENDIF"
      )
      expect(s.run).to be false
      expect(s).to be_invalid

      s = BitcoinOld::Script.from_string(
        "1 OP_IF 1 OP_ELSE #{BitcoinOld::Script::OPCODES[opcode]} 1 OP_ENDIF"
      )
      expect(s.run).to be false
      expect(s).to be_invalid
    end
  ...
  it 'check before casting and mark bad cases invalid' do
    ...
    s = BitcoinOld::Script.from_string('OP_NOT')
    expect(s.run).to be false
    expect(s).to be_invalid
  end

  it 'should do OP_CHECKSIGVERIFY and OP_CHECKMULTISIGVERIFY' do
    tx1 = BitcoinOld::P::Tx.new(
