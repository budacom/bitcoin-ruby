    ...
    context 'test vectors' do
      ...
      it 'works for valid checksums' do
        valid_checksums.each do |encoded_input|
          human_readable_part, data = BitcoinOld::Bech32.decode(encoded_input)
          result = BitcoinOld::Bech32.encode(human_readable_part, data)
      ...
      it 'returns nil invalid checksums' do
        invalid_checksums.each do |encoded_input|
          human_readable_part, = BitcoinOld::Bech32.decode(encoded_input)
    ...
    context 'test vectors' do
      it 'works for valid addresses' do
        valid_addresses.each do |test_address, _, test_script|
          ...
          version, program = BitcoinOld.decode_segwit_address(test_address)
          ...
          script = BitcoinOld::Script.to_witness_script(version, program)
          expect(script).to eq(test_script.pack('C*'))

          new_address = BitcoinOld.encode_segwit_address(version, program)
      ...
      it 'returns nil for invalid addresses' do
        invalid_addresses.each do |test_address|
          BitcoinOld.network = :bitcoin
          version, _program = BitcoinOld.decode_segwit_address(test_address)
      ...
      it 'returns nil for invalid address encodings' do
        invalid_address_encs.each do |test_hrp, test_version, test_length|
          BitcoinOld.network[:bech32_hrp] = test_hrp
          program_hex =
            Array.new(test_length) { 0 }.pack('C*').unpack('H*').first
          new_address = BitcoinOld.encode_segwit_address(test_version, program_hex)
