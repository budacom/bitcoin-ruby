...
describe 'BitcoinOld.network' do
  describe 'bitcoin network' do
    it 'returns the expected network descriptors' do
      BitcoinOld.network = :bitcoin

      expect(BitcoinOld.network[:magic_head]).to eq(['F9BEB4D9'].pack('H*'))
      expect(BitcoinOld.network[:address_version]).to eq('00')
      expect(BitcoinOld.network[:genesis_hash])
    ...
    it 'can be set to mainnet' do
      ...
      expect(BitcoinOld.network).to eq(BitcoinOld::NETWORKS[:bitcoin])
    end
  end
  ...
  describe 'magic head' do
    ...
    it 'uses the correct one for message parsing' do
      ...
      parser1 = BitcoinOld::Protocol::Parser.new(handler1)
      parser2 = BitcoinOld::Protocol::Parser.new(handler2)
    ...
    it 'uses the correct one for packet creation' do
      BitcoinOld.network = :testnet
      expect(BitcoinOld::Protocol.pkt('foo', 'bar')[0...4])
