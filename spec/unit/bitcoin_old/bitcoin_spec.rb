...
describe BitcoinOld do
  before { BitcoinOld.network = :bitcoin }
  ...
  describe '.hash160' do
    it 'produces the expected public key hash' do
      ...
      expect(BitcoinOld.hash160(pubkey))
  ...
  describe '.hash160_to_address' do
    ...
    context 'testnet' do
      before { BitcoinOld.network = :testnet }

      it 'produces the expected address' do
        expect(BitcoinOld.hash160_to_address(pubkey_hash))
  ...
  describe '.pubkey_to_address' do
    ...
    context 'testnet' do
      ...
      it 'works for compressed pubkey' do
        expect(BitcoinOld.pubkey_to_address(compressed_pubkey))
          .to eq('mu6vSuyvpVxvDAJyZczjxaU56pXLNBSf9C')
      end

      it 'works for uncompressed pubkey' do
        expect(BitcoinOld.pubkey_to_address(uncompressed_pubkey))
  ...
  describe '.pubkeys_to_p2sh_multisig_address' do
    ...
    context 'testnet' do
      ...
      it 'produces the expected address and redeem script' do
        address, redeem_script = BitcoinOld.pubkeys_to_p2sh_multisig_address(
          2, pubkey1, pubkey2
        )
  ...
  describe '.hash160_from_address' do
    ...
    context 'testnet' do
      ...
      it 'produces the expected hash' do
        expect(
          BitcoinOld.hash160_from_address('mpXwg4jMtRhuSpVq4xS3HFHmCmWp9NyGKt')
        ).to eq('62e907b15cbf27d5425399ebf6f0fb50ebb88f18')
      end

      it 'returns nil for invalid address' do
        expect(BitcoinOld.hash160_from_address('bad-address-testnet')).to be_nil
      end
      ...
      it 'survives rounds of conversion from hash160 to address' do
        ...
        expect(
          BitcoinOld.hash160_from_address(
            BitcoinOld.hash160_to_address(pubkey_hash_hex)
          )
        ).to eq(pubkey_hash_hex)

        expect(
          BitcoinOld.hash160_to_address(
            BitcoinOld.hash160_from_address(addr)
          )
        ).to eq(addr)
      end
    end
    ...
    context 'mainnet' do
      ...
      it 'returns expected values for addresses' do
        expect(
          BitcoinOld.hash160_from_address('1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa')
        ).to eq('62e907b15cbf27d5425399ebf6f0fb50ebb88f18')
        expect(
          BitcoinOld.hash160_from_address('11ofrrzv87Ls97jN4TUetfQp4gEsUSL7A')
        ).to eq('0026f5494b39ea04b7bcb05e583acf3b0102d61f')
        expect(
          BitcoinOld.hash160_from_address('11122RGUQSszAsTpptd2h8sdyYGR6nKs6f')
        ).to eq('0000daec8d6f05e949710f202c4f73258aa7791e')
        expect(
          BitcoinOld.hash160_from_address('11119uLoMQCBHmKevdsFKHMaUoyrwLa9Y')
        ).to eq('00000090c66372823859c935149e2e32d276a1e6')
        expect(
          BitcoinOld.hash160_from_address('1111136sgL8UNSTVL9ize2uGFPxFDGwFp')
      ...
      it 'returns nil for invalid address' do
        expect(BitcoinOld.hash160_from_address('bad-address-mainnet')).to be_nil
      end
  ...
  describe '.address_checksum?' do
    context 'testnet' do
      ...
      it 'returns true for valid addresses' do
        expect(
          BitcoinOld.address_checksum?('mpXwg4jMtRhuSpVq4xS3HFHmCmWp9NyGKt')
        ).to be true
        expect(
          BitcoinOld.address_checksum?('1D3KpY5kXnYhTbdCbZ9kXb2ZY7ZapD85cW')
        ).to be true
      end

      it 'returns false for invalid addresses' do
        expect(BitcoinOld.address_checksum?('f0f0f0')).to be false
      end
    end
    ...
    context 'mainnet' do
      ...
      it 'returns true for valid addresses' do
        expect(
          BitcoinOld.address_checksum?('1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa')
        ).to be true
        expect(
          BitcoinOld.address_checksum?('mpXwg4jMtRhuSpVq4xS3HFHmCmWp9NyGKt')
        ).to be true
  ...
  describe '.valid_address?' do
    context 'testnet' do
      ...
      it 'is true for valid addresses' do
        expect(
          BitcoinOld.valid_address?('mpXwg4jMtRhuSpVq4xS3HFHmCmWp9NyGKt')
        ).to be true
      end

      it 'is false for invalid addresses' do
        expect(
          BitcoinOld.valid_address?('1D3KpY5kXnYhTbdCbZ9kXb2ZY7ZapD85cW')
        ).to be false
        expect(
          BitcoinOld.valid_address?('1moYFpRM4LkTV4Ho5eCxiEPB2bSm3AsJNGj')
        ).to be false
        expect(BitcoinOld.valid_address?('f0f0f0')).to be false
      end

      it 'successfully validates a series of new addresses' do
        400.times do
          addr = BitcoinOld.generate_address
          expect(BitcoinOld.hash160_from_address(addr[0])).to eq(addr[-1])
          expect(BitcoinOld.hash160_to_address(addr[-1])).to eq(addr[0])
          expect(BitcoinOld.valid_address?(addr[0])).to be true
        end
      end

      it 'validates p2sh address' do
        expect(
          BitcoinOld.valid_address?('2MyLngQnhzjzatKsB7XfHYoP9e2XUXSiBMM')
        ).to be true
      end
    end
    ...
    context 'mainnet' do
      ...
      it 'is true for valid addresses' do
        ...
        expect(
          BitcoinOld.valid_address?('1ZQxJYBRmbb2rDNYPhd96x3eMbNnPD98q')
        ).to be true
        expect(
          BitcoinOld.valid_address?('12KhCL8nGK3Luy7ehU3AxPs1mTocdessLM')
        ).to be true
        expect(
          BitcoinOld.valid_address?('1AnNQgfaGgSKejzR6km74tyQPDGwZBBVT')
        ).to be true
      end
      ...
      it 'is false for invalid addresses' do
        ...
        expect(
          BitcoinOld.valid_address?('2D3KpY5kXnYhTbdCbZ9kXb2ZY7ZapD85cW')
        ).to be false
        expect(
          BitcoinOld.valid_address?('1D3KpY5kXnYhTbdCbZ9kXb2ZY7ZapD85cX')
        ).to be false
        expect(
          BitcoinOld.valid_address?('1moYFpRM4LkTV4Ho5eCxiEPB2bSm3AsJNGj')
        ).to be false
        expect(
          BitcoinOld.valid_address?('f0f0f0')
        ).to be false
      end
      ...
      it 'validates p2sh address' do
        expect(
          BitcoinOld.valid_address?('3CkxTG25waxsmd13FFgRChPuGYba3ar36B')
        ).to be true
      end
    end
  end
  ...
  describe '.base58_to_int' do
    it 'returns the expected integer equivalent' do
      expect(
        BitcoinOld.base58_to_int('114EpVhtPpJQKti8HiH2fvXZFPiPkgDZrE')
      ).to eq(
  ...
  describe '.valid_pubkey?' do
    let(:key) { BitcoinOld::Key.generate }

    it 'is true for compressed and uncompressed keys' do
      expect(BitcoinOld.valid_pubkey?(key.pub_compressed)).to be true
      expect(BitcoinOld.valid_pubkey?(key.pub_uncompressed)).to be true
    end

    it 'is false for invalid public keys' do
      expect(BitcoinOld.valid_pubkey?(key.addr)).to be false
      expect(BitcoinOld.valid_pubkey?(key.priv)).to be false
    end
  end
  ...
  describe '.address_type' do
    context 'testnet' do
      ...
      it 'works for a p2sh address' do
        expect(BitcoinOld.address_type('2MyLngQnhzjzatKsB7XfHYoP9e2XUXSiBMM'))
          .to eq(:p2sh)
      end

      it 'works for a hash160 address' do
        expect(BitcoinOld.address_type('mpXwg4jMtRhuSpVq4xS3HFHmCmWp9NyGKt'))
          .to eq(:hash160)
      end

      it 'is nil for an invalid address' do
        expect(BitcoinOld.address_type('1D3KpY5kXnYhTbdCbZ9kXb2ZY7ZapD85cW'))
          .to be_nil
      end
    end
    ...
    context 'mainnet' do
      ...
      it 'works for a p2sh address' do
        expect(BitcoinOld.address_type('3CkxTG25waxsmd13FFgRChPuGYba3ar36B'))
          .to eq(:p2sh)
      end
      ...
      it 'works for a witness_v0_keyhash address' do
        expect(BitcoinOld.address_type('bc1qw508d6qejxtdg4y5r3zarvary0c5xw7kv8f3t4'))
          .to eq(:witness_v0_keyhash)
      end

      it 'works for a witness_v0_scripthash address' do
        expect(
          BitcoinOld.address_type('bc1qrp33g0q5c5txsp9arysrx4k6zdkfs4nce4xj0gdcccefvpysxf3qccfmv3')
        ).to eq(:witness_v0_scripthash)
      end

      it 'is nil for invalid addresses' do
        expect(BitcoinOld.address_type('bc1qw508d6qejxtdg4y5r3zarvayr0c5xw7kv8f3t4'))
          .to be_nil
        expect(BitcoinOld.address_type('mpXwg4jMtRhuSpVq4xS3HFHmCmWp9NyGKt'))
          .to be_nil
      end
    ...
    context 'litecoin' do
      before { BitcoinOld.network = :litecoin }

      it 'works for p2sh addresses' do
        expect(
          BitcoinOld.address_type('3CkxTG25waxsmd13FFgRChPuGYba3ar36B')
        ).to eq(:p2sh)
        expect(
          BitcoinOld.address_type('MJy6m9S3thpJa8GwM8fm2LeJbFC22w18Vx')
        ).to eq(:p2sh)
      end

      it 'is nil for invalid addresses' do
        expect(
          BitcoinOld.address_type('2MyLngQnhzjzatKsB7XfHYoP9e2XUXSiBMM')
        ).to be_nil
      end
    end
    ...
    context 'zcash' do
      before {
        BitcoinOld::NETWORKS[:zcash] = BitcoinOld::NETWORKS[:bitcoin].merge(
          project: :zcash,
          address_version: '1cb8',
          p2sh_version: '1cbd',
        )
        BitcoinOld.network = :zcash
      }

      it 'works for a hash160 address' do
        expect(BitcoinOld.address_type('t1KBT8oCGAfisNNWnSD3h7TSsZ7qKah935g'))
          .to eq(:hash160)
      end
  ...
  describe '.checksum' do
    it 'produces the expected checksum' do
      expect(
        BitcoinOld.checksum('0062e907b15cbf27d5425399ebf6f0fb50ebb88f18')
      ).to eq('c29b7d93')
    end
  end
  ...
  describe '.bitcoin_mrkl / .bitcoin_hash' do
    ...
    it 'produces the expected values' do
      expect(BitcoinOld.bitcoin_hash(b + a)).to eq(c)
      expect(BitcoinOld.bitcoin_mrkl(a, b)).to eq(c)
    end
  end
  ...
  describe 'merkle trees' do
    ...
    it 'produces the expected tree for a simple tree' do
      expect(BitcoinOld.hash_mrkl_tree(merkle_tree1[0...2])).to eq(merkle_tree1)
    end

    it 'produces the expected tree for a larger tree' do
      expect(BitcoinOld.hash_mrkl_tree(merkle_tree2[0...3])).to eq(merkle_tree2)

      expect(
        BitcoinOld.bitcoin_mrkl(merkle_tree2[0], merkle_tree2[1])
      ).to eq(merkle_tree2[3])
      expect(
        BitcoinOld.bitcoin_mrkl(merkle_tree2[2], merkle_tree2[2])
      ).to eq(merkle_tree2[4])
      expect(
        BitcoinOld.bitcoin_mrkl(merkle_tree2[3], merkle_tree2[4])
      ).to eq(merkle_tree2[5])

      merkle_tree2[0...3].each.with_index do |target, idx|
        branch = BitcoinOld.hash_mrkl_branch(merkle_tree2[0...3], target)
        expect(
          BitcoinOld.mrkl_branch_root(branch, target, idx)
    ...
    it 'produces the expected results for a medium tree' do
      expect(
        BitcoinOld.bitcoin_mrkl(merkle_tree3[0], merkle_tree3[1])
      ).to eq(merkle_tree3[6])
      expect(
        BitcoinOld.bitcoin_mrkl(merkle_tree3[2], merkle_tree3[3])
      ).to eq(merkle_tree3[7])
      expect(
        BitcoinOld.bitcoin_mrkl(merkle_tree3[4], merkle_tree3[5])
      ).to eq(merkle_tree3[8])
      expect(
        BitcoinOld.bitcoin_mrkl(merkle_tree3[6], merkle_tree3[7])
      ).to eq(merkle_tree3[9])
      expect(
        BitcoinOld.bitcoin_mrkl(merkle_tree3[8], merkle_tree3[8])
      ).to eq(merkle_tree3[10])
      expect(
        BitcoinOld.bitcoin_mrkl(merkle_tree3[9], merkle_tree3[10])
      ).to eq(merkle_tree3[11])
      expect(
        BitcoinOld.hash_mrkl_tree(merkle_tree3[0...6])
      ).to eq(merkle_tree3)

      merkle_tree3[0...5].each.with_index do |target, idx|
        branch = BitcoinOld.hash_mrkl_branch(merkle_tree3[0..5], target)
        expect(
          BitcoinOld.mrkl_branch_root(branch, target, idx)
    ...
    it 'produces the expected results for a very large tree' do
      expect(
        BitcoinOld.bitcoin_mrkl(merkle_tree4[0], merkle_tree4[1])
      ).to eq(merkle_tree4[9])
      expect(
        BitcoinOld.bitcoin_mrkl(merkle_tree4[2], merkle_tree4[3])
      ).to eq(merkle_tree4[10])
      expect(
        BitcoinOld.bitcoin_mrkl(merkle_tree4[4], merkle_tree4[5])
      ).to eq(merkle_tree4[11])
      expect(
        BitcoinOld.bitcoin_mrkl(merkle_tree4[6], merkle_tree4[7])
      ).to eq(merkle_tree4[12])
      expect(
        BitcoinOld.bitcoin_mrkl(merkle_tree4[8], merkle_tree4[8])
      ).to eq(merkle_tree4[13])

      expect(
        BitcoinOld.bitcoin_mrkl(merkle_tree4[9], merkle_tree4[10])
      ).to eq(merkle_tree4[14])
      expect(
        BitcoinOld.bitcoin_mrkl(merkle_tree4[11], merkle_tree4[12])
      ).to eq(merkle_tree4[15])
      expect(
        BitcoinOld.bitcoin_mrkl(merkle_tree4[13], merkle_tree4[13])
      ).to eq(merkle_tree4[16])
      expect(
        BitcoinOld.bitcoin_mrkl(merkle_tree4[14], merkle_tree4[15])
      ).to eq(merkle_tree4[17])
      expect(
        BitcoinOld.bitcoin_mrkl(merkle_tree4[16], merkle_tree4[16])
      ).to eq(merkle_tree4[18])
      expect(
        BitcoinOld.bitcoin_mrkl(merkle_tree4[17], merkle_tree4[18])
      ).to eq(merkle_tree4[19])

      expect(BitcoinOld.hash_mrkl_tree(merkle_tree4[0...9])).to eq(merkle_tree4)

      merkle_tree4[0..8].each.with_index do |target, idx|
        branch = BitcoinOld.hash_mrkl_branch(merkle_tree4[0..8], target)
        expect(
          BitcoinOld.mrkl_branch_root(branch, target, idx)
    ...
    it 'does not allow duplicate hash in merkle trees' do
      expect(
        BitcoinOld.hash_mrkl_tree(%w[aa bb cc]).last
      ).not_to eq(BitcoinOld.hash_mrkl_tree(%w[aa bb cc cc]).last)
    end
    ...
    it 'returns a value even if a merkle branch is empty' do
      ...
      expect(
        BitcoinOld.mrkl_branch_root(branch.map(&:hth), target, mrkl_index)
      ).to eq(target)
    end
  end
  ...
  describe '.decode_compact_bits / .encode_compact_bits' do
    ...
    it 'decodes nonce compact bits to bignum hex' do
      ...
      # expect(BitcoinOld.decode_compact_bits('01fedcba'.to_i(16)).to_i(16)).to eq(-0x7e)
    end

    it 'encodes to the expected values' do
      expect(
        BitcoinOld.encode_compact_bits(BitcoinOld.decode_compact_bits(target1))
      ).to eq(target1)
      expect(
        BitcoinOld.encode_compact_bits(BitcoinOld.decode_compact_bits(target2))
      ).to eq(target2)
      expect(
        BitcoinOld.encode_compact_bits(BitcoinOld.decode_compact_bits(target3))
      ).to eq(target3)
    end
  end
  ...
  describe '.block_hash' do
    ...
    it 'produces the expected block hash' do
      expect(
        BitcoinOld.block_hash(prev_block0, merkle_root0, *time_bits_nonce_ver0)
      ).to eq(expected_block_hash0)
      expect(
        BitcoinOld.block_hash(prev_block1, merkle_root1, *time_bits_nonce_ver1)
      ).to eq(expected_block_hash1)
      expect(
        BitcoinOld.block_hash(prev_block169, merkle_root169, *time_bits_nonce_ver169)
      ).to eq(expected_block_hash169)
      expect(
        BitcoinOld.block_hash(prev_block171, merkle_root171, *time_bits_nonce_ver171)
  ...
  describe '.generate_key' do
    it 'generates an openssl-secp256k1 private/public keypair' do
      private_key, public_key = BitcoinOld.generate_key

      expect(private_key.size).to eq(64) # bytes in hex
      expect(public_key.size).to eq(130) # bytes in hex

      key = BitcoinOld.open_key(private_key, public_key)
      expect(BitcoinOld.inspect_key(key)).to eq([private_key, public_key])
    end
  end

  describe 'BitcoinOld::OpenSSL_EC' do
    it 'OpenSSL library is available' do
      expect do
        BitcoinOld::OpenSSL_EC
      end.not_to raise_error
    end

    describe 'BitcoinOld.open_key' do
      it 'opens key from private key and resolves public key' do
        50.times do
          private_key, public_key = BitcoinOld.generate_key
          key = BitcoinOld.open_key(private_key)
    ...
    describe 'signing and verifying messages' do
      context 'testnet' do
        before { BitcoinOld.network = :testnet3 }

        it 'verifies the signature of a testnet address' do
          expect(
            BitcoinOld.verify_message(
      ...
      context 'mainnet' do
        ...
        it 'successfully signs and verifies the message' do
          [address_and_keys1, address_and_keys2].each do |_addr, privkey, _pubkey|
            key = BitcoinOld.open_key(privkey)
            16.times.each do |count|
              signature = BitcoinOld.sign_message(
                key.private_key_hex,
                key.public_key_hex,
  ...
  describe '.generate_address' do
    it 'generates a new bitcoin address' do
      address, private_key, public_key, _hash160 = BitcoinOld.generate_address

      expect(private_key.size).to eq(64) # bytes in hex
      expect(public_key.size).to eq(130) # bytes in hex
      expect(
        BitcoinOld.hash160_to_address(BitcoinOld.hash160(public_key))
      ).to eq(address)
    end
  end
  ...
  describe '.encode_base58 / decode_base58' do
    it 'passes tests from bitcoin core' do
      ...
      fixtures.each do |hex, output|
        expect(BitcoinOld.encode_base58(hex)).to eq(output)
        expect(BitcoinOld.decode_base58(output)).to eq(hex)
      end
    end
  end

  it '.block_next_retarget' do
    expect(BitcoinOld.block_next_retarget(189_408)).to eq(189_503)
    expect(BitcoinOld.block_next_retarget(189_503)).to eq(189_503)
    expect(BitcoinOld.block_next_retarget(189_504)).to eq(191_519)
  end

  it '.block_difficulty' do
    expect(BitcoinOld.block_difficulty(436_835_377)).to eq('1751454.5353407')
  end
  ...
  describe '.block_new_target' do
    ...
    it 'should calculate retarget difficulty' do
      ...
      expect(
        BitcoinOld.decode_compact_bits(new_difficulty)
      ).to eq(BitcoinOld.decode_compact_bits(0x1a057e08))
    end
  end

  it '.block_hashes_to_win' do
    expect(BitcoinOld.block_hashes_to_win(436_835_377))
      .to eq(7_522_554_734_795_001)
  end

  it '.block_probability' do
    expect(BitcoinOld.block_probability(436_835_377))
      .to eq('0.0000000000000001329335625003267087884673003372881794348')
  end

  it '.block_average_hashing_time' do
    expect(
      BitcoinOld.block_average_hashing_time(436_835_377, 630_000_000)
    ).to eq(11_940_563)
  end

  it '.block_average_mining_time' do
    expect(
      BitcoinOld.block_average_mining_time(0x1a022fbe, 231_337, 270.0, 1.0)
  ...
  it '.blockchain_total_btc' do
    ...
    block_heights.zip(expected_results).map do |height, expected_result|
      expect(BitcoinOld.blockchain_total_btc(height)).to eq(expected_result)
    end
  end
  ...
  it '.block_creation_reward' do
    ...
    heights.zip(rewards).map do |height, reward|
      expect(BitcoinOld.block_creation_reward(height)).to eq(reward)
    end
  end
  ...
  describe 'bitcoin base58 test vectors' do
    ...
    it 'passes the valid keys cases' do
      valid_base58_keys.each do |test_case|
        ...
        computed_script = BitcoinOld::Script.to_address_script(address)
        expect(computed_script).to eq(script)

        expect(BitcoinOld.valid_address?(address.swapcase)).to eq(is_swapcase_valid)

        computed_address = BitcoinOld::Script.new(script).get_address
    ...
    it 'fails the invalid keys cases' do
      ...
      test_cases.each do |test_case|
        ...
        %i[bitcoin testnet3 regtest].each do |network_name|
          BitcoinOld.network = network_name
          expect(BitcoinOld.valid_address?(address)).to be false
        end
      end
    end
  end
