...
describe BitcoinOld::Key do
  ...
  let(:key) do
    BitcoinOld::Key.new(key_data[:priv], key_data[:pub], false)
  end

  it 'should generate a key' do
    k = BitcoinOld::Key.generate
    expect(k.priv.size).to eq(64)
    expect(k.pub.size).to eq(66)
    expect(k.compressed).to be true

    k = BitcoinOld::Key.generate(compressed: true)
    expect(k.priv.size).to eq(64)
    expect(k.pub.size).to eq(66)
    expect(k.compressed).to be true

    k = BitcoinOld::Key.generate(true)
    expect(k.priv.size).to eq(64)
    expect(k.pub.size).to eq(66)
    expect(k.compressed).to be true

    k = BitcoinOld::Key.generate(compressed: false)
    expect(k.priv.size).to eq(64)
    expect(k.pub.size).to eq(130)
    expect(k.compressed).to be false

    k = BitcoinOld::Key.generate(false)
    expect(k.priv.size).to eq(64)
    expect(k.pub.size).to eq(130)
    expect(k.compressed).to be false
  end
  ...
  it 'recovers public keys from compact signatures' do
    ...
    tests.each do |test|
      key = BitcoinOld::Key.recover_compact_signature_to_key(
  ...
  it 'should import private key in base58 format' do
    BitcoinOld.network = :bitcoin
    key = BitcoinOld::Key.from_base58(
  ...
  it 'should handle compressed and uncompressed pubkeys' do
    ...
    expect(BitcoinOld::Key.new(nil, compressed).compressed).to be true
    expect(BitcoinOld::Key.new(nil, compressed).pub).to eq(compressed)
    expect(BitcoinOld::Key.new(nil, compressed).addr)
      .to eq('1NdB761LmTmrJixxp93nz7pEiCx5cKPW44')
    expect(BitcoinOld::Key.new(nil, uncompressed).compressed).to be false
    expect(BitcoinOld::Key.new(nil, uncompressed).pub).to eq(uncompressed)
    expect(BitcoinOld::Key.new(nil, uncompressed).addr)
      .to eq('19FBCg9295EBQ4P6bSLTGyz2BdbbPcqQD')

    new_key = BitcoinOld::Key.new(nil, compressed)
    expect(new_key.pub_compressed).to eq(compressed)
    expect(new_key.pub_uncompressed).to eq(uncompressed)

    msg = 'foobar'
    sig = key.sign(msg)
    expect(BitcoinOld::Key.new(nil, key.pub_compressed).verify(msg, sig))
      .to be true
    expect(BitcoinOld::Key.new(nil, key.pub_uncompressed).verify(msg, sig))
      .to be true
  ...
  it 'should raise error for private key out of range.' do
    ...
    expect do
      BitcoinOld::Key.new('00')
    end.to raise_error(RuntimeError, 'private key is not on curve')

    BitcoinOld::Key.new(
      'FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEBAAEDCE6AF48A03BBFD25E8CD0364140'
    )
    BitcoinOld::Key.new('01')
  end
  ...
  describe 'BitcoinOld::OpenSSL_EC' do
    it 'resolves public from private key' do
      ...
      expect(BitcoinOld::OpenSSL_EC.regenerate_key(privkey))
      ...
      [
        ...
        expect(BitcoinOld::OpenSSL_EC.regenerate_key(privkey)).to eq(key)
      end

      250.times.each do
        keypair = BitcoinOld.generate_key
        expect(BitcoinOld::OpenSSL_EC.regenerate_key(keypair.first)).to eq(keypair)
      end
    end
    ...
    it 'recover public key from compact signature' do
      ...
      expect(BitcoinOld::OpenSSL_EC.recover_public_key_from_signature(*args))
        .to eq(expected)
    end
    ...
    it 'sign and verify text messages' do
      [
        ...
        k = BitcoinOld::Key.from_base58(privkey_base58)
        expect(k.compressed).to eq(expected_compression)
        k2 = BitcoinOld::Key.new(nil, k.pub)
        ...
        16.times do |n|
          ...
          expect(BitcoinOld::Key.verify_message(k.addr, signature, msg)).to be true
        end
      end
    end
  end
