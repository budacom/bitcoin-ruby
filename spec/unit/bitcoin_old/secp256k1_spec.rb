...
describe BitcoinOld::Secp256k1 do
  ...
  describe '#generate_key_pair' do
    ...
    it 'generates an uncompressed key pair' do
      private_key, public_key =
        BitcoinOld::Secp256k1.generate_key_pair(false)
  ...
  describe '#generate_key' do
    it 'generates a compressed key' do
      key = BitcoinOld::Secp256k1.generate_key(true)
      expect(key.compressed).to be true
    end

    it 'generates an uncompressed key' do
      key = BitcoinOld::Secp256k1.generate_key(false)
  ...
  describe '#sign' do
    it 'successfully signs and verifies text' do
      private_key, public_key = BitcoinOld::Secp256k1.generate_key_pair
      signature = BitcoinOld::Secp256k1.sign(fixture_text, private_key)

      expect(
        BitcoinOld::Secp256k1.verify(fixture_text, signature, public_key)
      ).to be true
      expect(
        BitcoinOld::Secp256k1.verify(fixture_text.upcase, signature, public_key)
      ).to be false
    end

    it 'supports RFC6979 deterministic signatures' do
      private_key, = BitcoinOld::Secp256k1.generate_key_pair
      first  = BitcoinOld::Secp256k1.sign(fixture_text, private_key)
      second = BitcoinOld::Secp256k1.sign(fixture_text, private_key)
  ...
  describe '#sign_compact' do
    it 'signs and recovers with compressed keys' do
      private_key, public_key = BitcoinOld::Secp256k1.generate_key_pair(true)

      signature = BitcoinOld::Secp256k1.sign_compact(
        fixture_text, private_key, true
      )
      expect(signature.bytesize).to eq(65)

      recovered_public_key = BitcoinOld::Secp256k1.recover_compact(
    ...
    it 'signs and recovers with uncompressed keys' do
      private_key, public_key = BitcoinOld::Secp256k1.generate_key_pair(false)
      signature = BitcoinOld::Secp256k1.sign_compact(
    ...
    context 'when compared to OpenSSL' do
      ...
      it 'has matching signing and recovery results' do
        ...
        openssl_signature = BitcoinOld::OpenSSL_EC.sign_compact(
          message, private_key_binary, nil, false
        )
        libsecp_signature = BitcoinOld::Secp256k1.sign_compact(
          message, private_key_binary, false
        )

        expect(
          BitcoinOld::OpenSSL_EC.recover_compact(message, openssl_signature)
        ).to eq(key.pub)
        expect(
          BitcoinOld::Secp256k1.recover_compact(message, openssl_signature)
            .unpack('H*')[0]
        ).to eq(key.pub)

        expect(
          BitcoinOld::OpenSSL_EC.recover_compact(message, libsecp_signature)
        ).to eq(key.pub)
        expect(
          BitcoinOld::Secp256k1.recover_compact(message, libsecp_signature)
            .unpack('H*')[0]
        ).to eq(key.pub)
      end
    end
