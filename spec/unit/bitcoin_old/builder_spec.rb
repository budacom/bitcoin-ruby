...
describe BitcoinOld::Builder do
  ...
  it 'should build transactions with input and output signatures' do
    ...
    expect(
      BitcoinOld::Script.new(tx.in[0].script_sig).chunks[1].unpack('H*')[0]
    ).to eq(keys[0].pub)

    expect(tx.out[0].value).to eq(123)
    script = BitcoinOld::Script.new(tx.out[0].pk_script)
    ...
    expect(
      BitcoinOld::Script.new(tx2.out[0].pk_script).to_string
    ).to eq(
  ...
  it 'should build transactions with p2wpkh signatures' do
    key = BitcoinOld::Key.new('619c335025c7f4012e556c2a58b2506e30b8511b53ade95ea316fd8c3286feb9')
  ...
  it 'should build p2sh transaction with p2wpkh signatures' do
    ...
    script_pubkey = BitcoinOld::Script.to_p2sh_script('bdbb096c26dd64dca06d0fbe8bb5e990ac3cdb42')
    tx = build_tx do |t|
      t.input do |i|
    ...
    expect(tx.in[0].script_sig).to eq(BitcoinOld::Script.pack_pushdata(witness_prog))
  end
  ...
  it 'should build unsigned transactions and add the signature hash' do
    ...
    expect(tx).to be_a(BitcoinOld::P::Tx)
  ...
  it 'should build unsigned multisig transactions and add the signature hash' do
    ...
    expect(tx2).to be_a(BitcoinOld::P::Tx)
  ...
  it 'should add change output' do
    change_address = BitcoinOld::Key.generate.addr
    ...
    expect(
      BitcoinOld::Script.new(tx.out.last.pk_script).get_address
  ...
  it 'should add change output and leave fee' do
    ...
    expect(tx.out.last.value)
      .to eq(50e8 - 12_345 - BitcoinOld.network[:min_tx_fee])
    expect(
      BitcoinOld::Script.new(tx.out.last.pk_script).get_address
    ).to eq(change_address)
  ...
  it 'randomize_outputs should not modify output values or fees' do
    ...
    expect(tx.out.count).to eq(2)
    expect(tx.out.last.value).to eq(50e8 - 12_345 - BitcoinOld.network[:min_tx_fee])
    expect(
      BitcoinOld::Script.new(tx.out.last.pk_script).get_address
    ).to eq(change_address)
  ...
  it 'should build op_return output' do
    builder = BitcoinOld::Builder::TxOutBuilder.new
  ...
  it 'should build op_return script' do
    ...
    expect(BitcoinOld::Script.new(result).to_string).to eq('OP_RETURN deadbeef')
  end
  ...
  it 'should build address script' do
    ...
    expect(BitcoinOld::Script.new(result).to_string)
      .to eq("OP_DUP OP_HASH160 #{BitcoinOld.hash160_from_address(key.addr)} " \
  ...
  it 'should build and spend multisig output' do
    ...
    expect(
      BitcoinOld::Script.new(tx1.out[0].pk_script).to_string
  ...
  it 'should build and spend p2sh multisig output' do
    ...
    expect(BitcoinOld::Script.new(tx1.out[0].pk_script).to_string)
      .to eq("OP_HASH160 #{BitcoinOld.hash160(tx1.out[0].redeem_script.hth)} OP_EQUAL")
    ...
    script = BitcoinOld::Script.new(tx2.in[0].script_sig, tx1.out[0].pk_script)
  ...
  it 'should build and sign bcash transaction' do
    ...
    expect(BitcoinOld::Script.new(tx.out[0].pk_script).to_string)
