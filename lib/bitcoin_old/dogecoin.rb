...
module BitcoinOld
  ...
  module Dogecoin
    def self.load
      BitcoinOld::Util.class_eval { include Util }
    end

    # fixed reward past the 600k block
    POST_600K_REWARD = 10_000 * BitcoinOld::COIN
      ...
      def self.included(base)
        ...
        base.class_eval do
          def block_creation_reward(block_height)
            reward_scaler = 2**(block_height / BitcoinOld.network[:reward_halving].to_f).floor
            if block_height < BitcoinOld.network[:difficulty_change_block]
              ...
              BitcoinOld.network[:reward_base] / reward_scaler * 2
            elsif block_height < 600_000
              BitcoinOld.network[:reward_base] / reward_scaler
            else
              POST_600K_REWARD
            end
          end
          ...
          def block_new_target(prev_height, prev_block_time, prev_block_bits, last_retarget_time)
            new_difficulty_protocol = (prev_height + 1) >= BitcoinOld.network[:difficulty_change_block]
            ...
            if new_difficulty_protocol
              ...
              retarget_time = BitcoinOld.network[:retarget_time_new]
            end
            ...
            if new_target < BitcoinOld.decode_compact_bits(
