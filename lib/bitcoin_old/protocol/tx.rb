  ...
  module Protocol
    ...
    class Tx
      ...
      def parse_data_from_io(data)
        ...
        if witness
          if BitcoinOld.litecoin? && (@flag & 8).positive?
      ...
      def signature_hash_for_input(
        ...
        if fork_id && (hash_type & SIGHASH_TYPE[:forkid]) != 0
          ...
          script_code = BitcoinOld::Protocol.pack_var_string(subscript)
      ...
      def signature_hash_for_witness_input(
        ...
        hash_type ||= SIGHASH_TYPE[:all]
        script = BitcoinOld::Script.new(witness_program)
        ...
        if script.is_witness_v0_keyhash?
          ...
          witness_pubkey_script_match = BitcoinOld::Script.to_witness_p2sh_script(
          ...
          script = if skip_separator_index > 0
                     s = BitcoinOld::Script.new(witness_script)
                     s.subscript_codeseparator(skip_separator_index)
                   else
                     witness_script
                   end
          script_code = BitcoinOld::Protocol.pack_var_string(script)
        end
      ...
      def verify_input_signature(in_idx, outpoint_data, block_timestamp = Time.now.to_i, opts = {})
        ...
        @scripts[in_idx] = BitcoinOld::Script.new(script_sig, script_pubkey)
        ...
        sig_valid = @scripts[in_idx].run(
          ...
          BitcoinOld.verify_signature(hash, sig, pubkey.unpack('H*')[0])
        end
        # BIP62 rule #6
      ...
      def verify_witness_input_signature(
        ...
        script_pubkey = script_pubkey_from_outpoint_data(outpoint_data, outpoint_idx)
        script_pubkey = BitcoinOld::Script.new(script_pubkey)

        if script_pubkey.is_p2sh?
          redeem_script = BitcoinOld::Script.new(@in[in_idx].script_sig).get_pubkey

          # P2SH-P2WPKH or P2SH-P2WSH
          script_pubkey = if BitcoinOld.hash160(redeem_script) == script_pubkey.get_hash160
                            BitcoinOld::Script.new(redeem_script.htb)
                          end
        end

        @in[in_idx].script_witness.stack.each { |s| script_sig << BitcoinOld::Script.pack_pushdata(s) }
        code_separator_index = 0

        if script_pubkey.is_witness_v0_keyhash? # P2WPKH
          @scripts[in_idx] = BitcoinOld::Script.new(
            script_sig, BitcoinOld::Script.to_hash160_script(script_pubkey.get_hash160)
          )
        elsif script_pubkey.is_witness_v0_scripthash? # P2WSH
          witness_hex = @in[in_idx].script_witness.stack.last.bth
          witness_script = BitcoinOld::Script.new(witness_hex.htb)
          return false unless BitcoinOld.sha256(witness_hex) == script_pubkey.get_hash160
          @scripts[in_idx] = BitcoinOld::Script.new(
            script_sig, BitcoinOld::Script.to_p2sh_script(BitcoinOld.hash160(witness_hex))
          )
        else
          return false
        end
      ...
      def bitcoinconsensus_verify_script(
        ...
        consensus_available = BitcoinOld::BitcoinConsensus.lib_available?
        raise 'BitcoinOld::BitcoinConsensus shared library not found' unless consensus_available

        outpoint_idx  = @in[in_idx].prev_out_index
        script_pubkey = script_pubkey_from_outpoint_data(outpoint_data, outpoint_idx)

        flags  = BitcoinOld::BitcoinConsensus::SCRIPT_VERIFY_NONE
        flags |= BitcoinOld::BitcoinConsensus::SCRIPT_VERIFY_P2SH if block_timestamp >= 1_333_238_400
        flags |= BitcoinOld::BitcoinConsensus::SCRIPT_VERIFY_SIGPUSHONLY if opts[:verify_sigpushonly]
        flags |= BitcoinOld::BitcoinConsensus::SCRIPT_VERIFY_MINIMALDATA if opts[:verify_minimaldata]
        flags |= BitcoinOld::BitcoinConsensus::SCRIPT_VERIFY_CLEANSTACK if opts[:verify_cleanstack]
        flags |= BitcoinOld::BitcoinConsensus::SCRIPT_VERIFY_LOW_S if opts[:verify_low_s]

        payload ||= to_payload
        BitcoinOld::BitcoinConsensus.verify_script(in_idx, script_pubkey, payload, flags)
      end
      ...
      def self.from_file(path)
        new(BitcoinOld::Protocol.read_binary_file(path))
      end

      # read json block from a file
      def self.from_json_file(path)
        from_json(BitcoinOld::Protocol.read_binary_file(path))
      end
      ...
      def final?(block_height, block_time)
        ...
        lock_threshold = lock_time < BitcoinOld::LOCKTIME_THRESHOLD ? block_height : block_time
      ...
      def legacy_sigops_count
        ...
        self.in.each do |txin|
          count += BitcoinOld::Script.new(txin.script_sig).sigops_count_accurate(false)
        end
        out.each do |txout|
          count += BitcoinOld::Script.new(txout.pk_script).sigops_count_accurate(false)
        end
        count
      end
      ...
      def calculate_minimum_fee(allow_free = true, mode = :block)
        ...
        base_fee = if mode == :relay
                     BitcoinOld.network[:min_relay_tx_fee]
                   else
                     BitcoinOld.network[:min_tx_fee]
                   end
        ...
        if allow_free
          ...
          min_free_size = if mode == :block
                            BitcoinOld.network[:free_tx_bytes]
                          else
        ...
        if min_fee < base_fee && mode == :block
          outputs.each do |output|
            if output.value < BitcoinOld.network[:dust]
              ...
              if BitcoinOld.network[:per_dust_fee]
                min_fee += base_fee
              else
                min_fee = base_fee
                break
        ...
        min_fee = BitcoinOld.network[:max_money] unless min_fee.between?(
          0, BitcoinOld.network[:max_money]
        )
        min_fee
      end
