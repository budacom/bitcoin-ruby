...
module BitcoinOld
  module Protocol
    ...
    class TxIn
      ...
      def parsed_script
        @parsed_script ||= BitcoinOld::Script.new(script_sig)
      end
      ...
      def to_hash(_options = {})
        ...
        if coinbase?
          ...
          t['scriptSig'] = BitcoinOld::Script.new(@script_sig).to_string
        end
      ...
      def add_signature_pubkey_script(sig, pubkey_hex)
        self.script = BitcoinOld::Script.to_signature_pubkey_script(sig, [pubkey_hex].pack('H*'))
      end
    end
  end
end
