  ...
  module Protocol
    ...
    class Block
      ...
      def parse_data_from_io(buf, header_only = false)
        ...
        if !BitcoinOld.network[:auxpow_chain_id].nil? && (@ver & BLOCK_VERSION_AUXPOW) > 0
      ...
      def recalc_mrkl_root
        @mrkl_root = if partial_merkle_tree
                       ...
                       BitcoinOld.hash_mrkl_tree(@tx.map(&:hash)).last.htb_reverse
                     end
      end
      ...
      def verify_mrkl_root
        if partial_merkle_tree
          ...
          @mrkl_root.reverse_hth == BitcoinOld.hash_mrkl_tree(@tx.map(&:hash)).last
        end
      end
      ...
      def to_hash(options = {})
        h = {
          ...
          'mrkl_tree' => BitcoinOld.hash_mrkl_tree(@tx.map(&:hash))
        }
      ...
      def hextarget
        BitcoinOld.decode_compact_bits(@bits)
      end

      def decimaltarget
        BitcoinOld.decode_compact_bits(@bits).to_i(16)
      end

      def difficulty
        BitcoinOld.block_difficulty(@bits)
      end
      ...
      def block_work
        target = BitcoinOld.decode_compact_bits(@bits).to_i(16)
