module BitcoinOld
  ...
  module ContractHash
    ...
    def self.generate(redeem_script_hex, payee_address_or_ascii, nonce_hex = nil)
      redeem_script = BitcoinOld::Script.new([redeem_script_hex].pack('H*'))
      ...
      redeem_script.get_multisig_pubkeys.each do |pubkey|
        ...
        key = BitcoinOld::Key.new(nil, pubkey.unpack('H*')[0])
        ...
        key = BitcoinOld::Key.new(nil, point.to_s(16))
        key.instance_eval { @pubkey_compressed = true }
        derived_keys << key.pub
      end

      m = redeem_script.get_signatures_required
      p2sh_script, redeem_script = BitcoinOld::Script.to_p2sh_multisig_script(m, *derived_keys)

      [nonce_hex, redeem_script.unpack('H*')[0], BitcoinOld::Script.new(p2sh_script).get_p2sh_address]
    end

    # claim a contract
    def self.claim(private_key_wif, payee_address_or_ascii, nonce_hex)
      key = BitcoinOld::Key.from_base58(private_key_wif)
      ...
      BitcoinOld::Key.new(derived_key.to_s(16))
    end

    # compute HMAC data
    def self.compute_data(address_or_ascii, nonce_hex)
      nonce = nonce_hex ? [nonce_hex].pack('H32') : SecureRandom.random_bytes(16)
      if BitcoinOld.valid_address?(address_or_ascii)
        address_type = case BitcoinOld.address_type(address_or_ascii)
                       when :hash160 then  'P2PH'
                       when :p2sh then     'P2SH'
                       else
                         raise "unsupported address type #{address_type}"
                       end
        contract_bytes = [BitcoinOld.hash160_from_address(address_or_ascii)].pack('H*')
      else
        address_type = 'TEXT'
