...
module BitcoinOld
  ...
  class Key
    ...
    def self.from_base58(str)
      hex = BitcoinOld.decode_base58(str)
      compressed = hex.size == 76
      version, key, flag, checksum = hex.unpack("a2a64a#{compressed ? 2 : 0}a8")
      raise "Invalid version"   unless version == BitcoinOld.network[:privkey_version]
      raise "Invalid checksum"  unless BitcoinOld.checksum(version + key + flag) == checksum
    ...
    def hash160
      BitcoinOld.hash160(pub)
    end

    # Get the address corresponding to the public key.
    def addr
      BitcoinOld.hash160_to_address(hash160)
    end

    # Sign +data+ with the key.
    #  key1 = BitcoinOld::Key.generate
    #  sig = key1.sign("some data")
    def sign(data)
      BitcoinOld.sign_data(key, data)
    end

    # Verify signature +sig+ for +data+.
    #  key2 = BitcoinOld::Key.new(nil, key1.pub)
    #  key2.verify("some data", sig)
    def verify(data, sig)
      regenerate_pubkey unless @key.public_key
      sig = BitcoinOld::OpenSSL_EC.repack_der_signature(sig)
      if sig
    ...
    def sign_message(message)
      BitcoinOld.sign_message(priv, pub, message)['signature']
    end

    def verify_message(signature, message)
      BitcoinOld.verify_message(addr, signature, message)
    end

    def self.verify_message(address, signature, message)
      BitcoinOld.verify_message(address, signature, message)
    end
    ...
    def self.recover_compact_signature_to_key(data, signature_base64)
      ...
      hash = BitcoinOld.bitcoin_signed_message_hash(data)
      pub_hex = BitcoinOld::OpenSSL_EC.recover_public_key_from_signature(hash, signature, version-27, compressed)
    ...
    def to_base58
      data = BitcoinOld.network[:privkey_version] + priv
      data += "01"  if @pubkey_compressed
      hex  = data + BitcoinOld.checksum(data)
      BitcoinOld.int_to_base58( hex.to_i(16) )
    end
    ...
    def to_bip38(passphrase)
      ...
      encrypted_privkey = BitcoinOld.encode_base58( encrypted_privkey.unpack("H*")[0] )
    end
    ...
    def self.from_bip38(encrypted_privkey, passphrase)
      version, flagbyte, addresshash, encryptedhalf1, encryptedhalf2, checksum =
        [ BitcoinOld.decode_base58(encrypted_privkey) ].pack("H*").unpack("a2aa4a16a16a4")
      ...
      key = BitcoinOld::Key.new(priv, nil, compressed)
    ...
    def self.from_warp(passphrase, salt="", compressed=false)
      ...
      key = BitcoinOld::Key.new(s3.unpack("H*")[0], nil, compressed)
    ...
    def regenerate_pubkey
      return nil unless @key.private_key
      set_pub(BitcoinOld::OpenSSL_EC.regenerate_key(priv)[1], @pubkey_compressed)
    end
