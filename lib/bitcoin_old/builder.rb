...
module BitcoinOld
  ...
  module Builder
    # build a BitcoinOld::Protocol::Block matching the given +target+.
    ...
    # build a BitcoinOld::Protocol::Tx.
    ...
    # DSL to create a BitcoinOld::Protocol::Block used by Builder#create_block.
    ...
    #  end
    #
    # See BitcoinOld::Builder::TxBuilder for details on building transactions.
    class BlockBuilder
      ...
      def block(target)
        ...
        @block.nonce = 0
        @block.mrkl_root = BitcoinOld.hash_mrkl_tree(@block.tx.map(&:hash)).last.htb.reverse
        find_hash(target)
      ...
      def find_hash(target)
        @block.bits = BitcoinOld.encode_compact_bits(target)
        t = Time.now
    ...
    # DSL to create BitcoinOld::Protocol::Tx used by Builder#build_tx.
    #  tx = tx do |t|
    #    t.input do |i|
    ...
    class TxBuilder
      ...
      def sig_hash_and_all_keys_exist?(inc, sig_script)
        ...
        script = BitcoinOld::Script.new(sig_script)
        return true if script.is_hash160? ||
                       script.is_pubkey? ||
                       script.is_witness_v0_keyhash? ||
                       (BitcoinOld.namecoin? && script.is_namecoin?)
      ...
      def sign_input(i, inc)
        ...
        if sig_hash_and_all_keys_exist?(inc, sig_script)
          ...
          if script.is_witness_v0_keyhash? # for p2wpkh
            ...
            redeem_script = inc.instance_eval { @redeem_script }
            @tx.in[i].script_sig = BitcoinOld::Script.pack_pushdata(redeem_script) if redeem_script
          else
    ...
    # Create a BitcoinOld::Protocol::TxIn used by TxBuilder#input.
    #
    ...
    #    i.signature_key key # BitcoinOld::Key used to sign the input
    #  end
    #
    ...
    class TxInBuilder
      ...
      # BitcoinOld::Key used to sign the signature_hash for the input.
      # see BitcoinOld::Script.signature_hash_for_input and BitcoinOld::Key.sign.
    ...
    # Create a BitcoinOld::Script used by TxOutBuilder#script.
    class ScriptBuilder
    ...
    # Create a BitcoinOld::Protocol::TxOut used by TxBuilder#output.
    #
    ...
    class TxOutBuilder
      ...
      def to(recipient, type = :address)
        @txout.pk_script, @txout.redeem_script = *BitcoinOld::Script.send(
