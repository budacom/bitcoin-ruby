...
module BitcoinOld
  ...
  class ExtKey
    ...
    def self.generate_master(seed)
      ...
      l = BitcoinOld.hmac_sha512('Bitcoin seed', seed)
      left = OpenSSL::BN.from_hex(l[0..31].bth).to_i
      raise 'invalid key' if left >= CURVE_ORDER || left == 0
      key.priv_key = BitcoinOld::Key.new(l[0..31].bth)
    ...
    def ext_pubkey
      ...
      key = BitcoinOld::Key.new(nil, priv_key.pub, compressed: true)
    ...
    def to_payload
      BitcoinOld.network[:extended_privkey_version].htb << [depth].pack('C') << parent_fingerprint.htb << [number].pack('N') << chain_code << [0x00].pack('C') << priv_key.priv.htb
    end

    # Base58 encoded extended private key
    def to_base58
      h = to_payload.bth
      hex = h + BitcoinOld.checksum(h)
      BitcoinOld.encode_base58(hex)
    end
    ...
    # get key identifier
    def identifier
      BitcoinOld.hash160(priv_key.pub)
    end
    ...
    def derive(number)
      ...
      l = BitcoinOld.hmac_sha512(chain_code, data)
      left = OpenSSL::BN.from_hex(l[0..31].bth).to_i
      ...
      new_key.priv_key = BitcoinOld::Key.new(child_priv.to_hex.rjust(64, '0'))
    ...
    def self.from_base58(address)
      data = StringIO.new(BitcoinOld.decode_base58(address).htb)
      key = ExtKey.new
      ...
      data.read(1) # 0x00
      key.priv_key = BitcoinOld::Key.new(data.read(32).bth)
      key
    end
  ...
  class ExtPubkey
    ...
    def to_payload
      BitcoinOld.network[:extended_pubkey_version].htb << [depth].pack('C') << parent_fingerprint.htb << [number].pack('N') << chain_code << pub.htb
    end
    ...
    # get address
    def addr
      BitcoinOld.hash160_to_address(BitcoinOld.hash160(pub))
    end
    ...
    def derive(number)
      ...
      new_key.pub_key = BitcoinOld.bitcoin_elliptic_curve.group.generator.mul(left).ec_add(pub_key)
      new_key.chain_code = l[32..-1]
      new_key
    end
    ...
    def self.from_base58(address)
      ...
      key.number = data.read(4).unpack('N').first
      key.chain_code = data.read(32)
      key.pub_key = OpenSSL::PKey::EC::Point.from_hex(BitcoinOld.bitcoin_elliptic_curve.group, data.read(33).bth)
      key
    end
  end
