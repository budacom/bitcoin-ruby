...
class BitcoinOld::Script
  ...
  def initialize(input_script, previous_output_script=nil)
    ...
    @raw = if @previous_output_script
             @input_script + [ BitcoinOld::Script::OP_CODESEPARATOR ].pack("C") + @previous_output_script
           else
             @input_script
           end
    ...
    if previous_output_script
      ...
      @chunks << BitcoinOld::Script::OP_CODESEPARATOR
  ...
  def to_binary(chunks=nil)
    ...
    (chunks || @chunks).map{|chunk|
      case chunk
      when BitcoinOld::Integer; [chunk].pack("C*")
  ...
  def append_opcode(opcode)
    raise "Opcode should be an integer" if !opcode.is_a?(BitcoinOld::Integer)
  ...
  def self.binary_from_string(script_string)
    ...
    script_string.split(" ").each{|i|
      ...
      buf << if i.is_a?(BitcoinOld::Integer)
      ...
      when BitcoinOld::Integer
        ...
        when *OPCODES_METHOD.keys
          ...
          when -2 # One fixed parameter, one optional
            ...
            puts "BitcoinOld::Script: opcode #{name} method parameters invalid"
          end
        when *OP_2_16
          ...
          puts "BitcoinOld::Script: opcode #{name} unkown or not implemented\n#{to_string.inspect}"
  ...
  def pay_to_script_hash(block_timestamp, opts, check_callback)
    ...
    *rest, script, _, script_hash, _ = @chunks
    ...
    return false unless BitcoinOld.hash160(script.unpack("H*")[0]) == script_hash.unpack("H*")[0]
  ...
  def inner_p2sh_script
    ...
    *rest, script, _, script_hash, _ = @chunks
    ...
    return nil unless BitcoinOld.hash160(script.unpack("H*")[0]) == script_hash.unpack("H*")[0]
    script
  end
  ...
  def is_pay_to_script_hash?
    ...
    if @previous_output_script
      chunks = BitcoinOld::Script.new(@previous_output_script).chunks
      chunks.size == 3 &&
      chunks[-3] == OP_HASH160 &&
  ...
  def is_multisig?
    return false  if @chunks.size < 4 || !@chunks[-2].is_a?(BitcoinOld::Integer)
  ...
  def get_pubkey_address
    BitcoinOld.pubkey_to_address(get_pubkey)
  end
  ...
  def get_hash160
    ...
    return BitcoinOld.hash160(get_pubkey)        if is_pubkey?
  ...
  def get_hash160_address
    BitcoinOld.hash160_to_address(get_hash160)
  end
  ...
  def get_multisig_addresses
    get_multisig_pubkeys.map{|pub|
      begin
        BitcoinOld::Key.new(nil, pub.unpack("H*")[0]).addr
  ...
  def get_p2sh_address
    BitcoinOld.hash160_to_p2sh_address(get_hash160)
  end
  ...
  def get_addresses
    ...
    if is_witness_v0_keyhash? || is_witness_v0_scripthash?
      ...
      return [BitcoinOld.encode_segwit_address(0, program_hex)]
    end
  ...
  def self.to_p2sh_multisig_script(*args)
    ...
    p2sh_script = to_p2sh_script(BitcoinOld.hash160(redeem_script.hth))
  ...
  def self.sort_p2sh_multisig_signatures script_sig, sig_hash
    ...
    sigs = Hash[script.chunks[1...-1].map.with_index do |sig, idx|
      pubkey = pubkeys.map {|key|
        BitcoinOld::Key.new(nil, key.hth).verify(sig_hash, sig) ? key : nil }.compact.first
  ...
  def codeseparator_count
    @chunks.select{|c|c == BitcoinOld::Script::OP_CODESEPARATOR}.length
  end
  ...
  def sigops_count_accurate(is_accurate)
    ...
    @chunks.each do |chunk| # pushdate or opcode
      if chunk == OP_CHECKSIG || chunk == OP_CHECKSIGVERIFY
        ...
        if is_accurate && last_opcode && last_opcode.is_a?(BitcoinOld::Integer) && last_opcode >= OP_1 && last_opcode <= OP_16
          count += ::BitcoinOld::Script.decode_OP_N(last_opcode)
        else
          count += 20
        end
      end
  ...
  def sigops_count_for_p2sh
    ...
    ::BitcoinOld::Script.new(data).sigops_count_accurate(true)
  end
  ...
  def self.decode_OP_N(opcode)
    ...
    if opcode.is_a?(BitcoinOld::Integer) && opcode >= OP_1 && opcode <= OP_16
  ...
  def codehash_script(opcode)
    ...
    checkhash = BitcoinOld.hash160(BitcoinOld::Script.binary_from_string(script).unpack("H*")[0])
    [script, checkhash]
  end
  ...
  def op_checksig(check_callback, opts={})
    ...
    return (@stack << 0) unless BitcoinOld::Script.check_pubkey_encoding?(pubkey, opts)
    drop_sigs      = [ cast_to_string(@stack[-1]) ]

    signature = cast_to_string(@stack.pop)
    return invalid unless BitcoinOld::Script.check_signature_encoding?(signature, opts)
  ...
  def sighash_subscript(drop_sigs, opts = {})
    ...
    if inner_p2sh? && @inner_script_code
      ::BitcoinOld::Script.new(@inner_script_code).to_binary_without_signatures(drop_sigs)
    else
  ...
  def op_checkmultisig(check_callback, opts={})
    ...
    while success && n_sigs > 0
      ...
      return (@stack << 0) unless BitcoinOld::Script.check_pubkey_encoding?(pub, opts)
      return invalid unless BitcoinOld::Script.check_signature_encoding?(sig, opts)
