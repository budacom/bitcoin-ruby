...
module BitcoinOld
  ...
  module ConnectionHandler
    ...
    def on_handshake_begin
      block   = 127_953
      from    = '127.0.0.1:8333'
      from_id = BitcoinOld::Protocol::Uniq
  ...
  class Connection < EM::Connection
    ...
    def self.connect_random_from_dns(connections)
      seeds = BitcoinOld.network[:dns_seeds]
      if seeds.empty?
      ...
      connect(host, BitcoinOld.network[:default_port], connections)
    end
  end
end
...
if $PROGRAM_NAME == __FILE__
  EM.run do
    connections = []
    # BitcoinOld::Connection.connect('127.0.0.1', 8333, connections)
    # BitcoinOld::Connection.connect('217.157.1.202', 8333, connections)
    BitcoinOld::Connection.connect_random_from_dns(connections)
  end
end
