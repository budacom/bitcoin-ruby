...
module BitcoinOld
  ...
  autoload :Bech32,     'bitcoin_old/bech32'
  autoload :Connection, 'bitcoin_old/connection'
  autoload :Protocol,   'bitcoin_old/protocol'
  autoload :P,          'bitcoin_old/protocol'
  autoload :Script,     'bitcoin_old/script'
  autoload :VERSION,    'bitcoin_old/version'
  autoload :Key,        'bitcoin_old/key'
  autoload :ExtKey,     'bitcoin_old/ext_key'
  autoload :ExtPubkey,  'bitcoin_old/ext_key'
  autoload :Builder,    'bitcoin_old/builder'
  autoload :BloomFilter,'bitcoin_old/bloom_filter'

  autoload :Dogecoin,   'bitcoin_old/dogecoin'
  autoload :Litecoin,   'bitcoin_old/litecoin'

  autoload :ContractHash,   'bitcoin_old/contracthash'

  module Trezor
    autoload :Mnemonic,   'bitcoin_old/trezor/mnemonic'
  end


  module Util

    def address_version; BitcoinOld.network[:address_version]; end
    def version_bytes; address_version.size / 2; end
    def p2sh_version; BitcoinOld.network[:p2sh_version]; end
    ...
    def address_type(address)
      ...
      if hex && hex.bytesize == target_size && address_checksum?(address)
        ...
        if BitcoinOld.network[:legacy_p2sh_versions]
          p2sh_versions += BitcoinOld.network[:legacy_p2sh_versions]
        end
    ...
    def pubkeys_to_p2sh_multisig_address(m, *pubkeys)
      redeem_script = BitcoinOld::Script.to_p2sh_multisig_script(m, *pubkeys).last
      return BitcoinOld.hash160_to_p2sh_address(BitcoinOld.hash160(redeem_script.hth)), redeem_script
    end


    def encode_segwit_address(version, program_hex)
      hrp = BitcoinOld.network[:bech32_hrp]
      ...
      data = [ version ] + BitcoinOld::Bech32.convert_bits(program.unpack("C*"), from_bits: 8, to_bits: 5, pad: true)

      address = BitcoinOld::Bech32.encode(hrp, data)
    ...
    def decode_segwit_address(address)
      ...
      actual_hrp, data  = BitcoinOld::Bech32.decode(address)
      ...
      program = BitcoinOld::Bech32.convert_bits(data[1..-1], from_bits: 5, to_bits: 8, pad: false)
    ...
    def decode_compact_bits(bits)
      if BitcoinOld.network_project == :dogecoin
    ...
    def sign_data(key, data)
      ...
      loop {
        ...
        sig = if Script.is_low_der_signature?(sig)
                ...
                BitcoinOld::OpenSSL_EC.signature_to_low_s(sig)
              end
        ...
        if Script.is_der_signature?(buf)
          ...
          p ["BitcoinOld#sign_data: invalid der signature generated, trying again.", data.unpack("H*")[0], sig.unpack("H*")[0]]
        end
      }
      return sig
    end
    ...
    def verify_signature(hash, signature, public_key)
      ...
      signature = BitcoinOld::OpenSSL_EC.repack_der_signature(signature)
      if signature
    ...
    def bitcoin_signed_message_hash(message)
      ...
      magic = BitcoinOld.network[:message_magic]
    ...
    def block_difficulty(target_nbits)
      ...
      # current_target  = BitcoinOld.decode_compact_bits(target_nbits).to_i(16)
    ...
    def block_new_target(prev_height, prev_block_time, prev_block_bits, last_retarget_time)
      ...
      retarget_time = BitcoinOld.network[:retarget_time]
      ...
      if new_target < BitcoinOld.decode_compact_bits(BitcoinOld.network[:proof_of_work_limit]).to_i(16)
        encode_compact_bits(new_target.to_s(16))
      else
        BitcoinOld.network[:proof_of_work_limit]
      end
    end
    ...
    def blockchain_total_btc(height)
      reward, interval = BitcoinOld.network[:reward_base], BitcoinOld.network[:reward_halving]
      total_btc = reward
    ...
    def block_creation_reward(block_height)
      BitcoinOld.network[:reward_base] / (2 ** (block_height / BitcoinOld.network[:reward_halving].to_f).floor)
    end
  end
  ...
  class ::String
    include BitcoinOld::BinaryExtensions
  end
  ...
  autoload :OpenSSL_EC, 'bitcoin_old/ffi/openssl'

  autoload :Secp256k1, 'bitcoin_old/ffi/secp256k1'
  autoload :BitcoinConsensus, 'bitcoin_old/ffi/bitcoinconsensus'
